...
public class TNManager : MonoBehaviour
{
    ...
    public System.Collections.Generic.List<GameObject> objects = new System.Collections.Generic.List<GameObject>();
	...
	public static void Disconnect () { if (mInstance != null) mInstance.mClient.Disconnect(); }
...
#region MonoBehaviour and helper functions -- it's unlikely that you will need to modify these
	...
	static public int IndexOf (GameObject go)
	{
		...
		{
			for (int i = 0, imax = mInstance.objects.Count; i < imax; ++i)
	...
	void OnCreateObject (int creator, int index, uint objectID, BinaryReader reader)
	{
		...
		else if (index >= 0 && index < objects.Count)
		{
